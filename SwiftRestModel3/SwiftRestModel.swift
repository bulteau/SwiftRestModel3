//
//  SwiftRestModel.swift
//  Hyphen Pro
//
//  Created by François Bulteau on 20/09/2016.
...
public class SwiftRestModel: NSObject
{
    ...
    /**
     ...
     - parameter error  : Error handler callback. `nil` by default.
     */
    public func destroy(success: ((_ response: JSON) -> ())? = nil, error: ((_ response: JSON) -> ())? = nil) {
        if (!self.isNew()) {
    ...
    /**
     ...
     - parameter error   : Error handler callback. `nil` by default.
     */
    public func request(method:String = "get", url: String = "", data parameters: Dictionary<String, AnyObject> = [:], headers: Dictionary<String, String> = [:], encoding: ParameterEncoding = Alamofire.URLEncoding(), success: ((_ response: JSON) -> ())? = nil, error: ((_ response: JSON) -> ())? = nil) {
